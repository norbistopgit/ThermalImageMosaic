...
class FullscreenOverlayApp:
    ...
    def show_about(self):
        messagebox.showinfo(
            ...
            "This tool allows users to visually align and merge RGB and thermal images into mosaics.\n"
    ...
    def place_image(self, path, zone='top'):
        try:
            from constants import MAX_IMG_WIDTH, MAX_IMG_HEIGHT
            img = Image.open(path).convert("RGBA")
            img = resize_image_keep_aspect(img, MAX_IMG_WIDTH, MAX_IMG_HEIGHT)
    ...
    def bind_drag_and_select(self, item):
        ...
        def on_right_click(event, item=item):
        ...
        self.canvas.tag_bind(item, "<Button-3>", on_right_click)
    ...
    def redraw_border(self, item):
        ...
        if not bbox:
            return
        x1, y1, x2, y2 = bbox
        border = self.canvas.create_rectangle(x1, y1, x2, y2, outline="red", width=2, tags=("border", f"border_{item}"))
    ...
    def show_edit_window(self, item):
        ...
        top.transient(self.root)              # not fullscreen
        top.resizable(True, True)             # resize
        top.overrideredirect(False)           # move
    ...
    def open_help_pdf(self):
        try:
            if os.path.exists(HELP_PDF_PATH):
                ...
                print("No PDF file found:", HELP_PDF_PATH)
        except Exception as e:
            print(f"Error open HELP file: {e}")
